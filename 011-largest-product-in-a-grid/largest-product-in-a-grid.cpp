...
int main()
{
	...
	int gridMatrix[ROW_SIZE][COL_SIZE];
	...
	while (i < GRID.length())
	{
		for (int j = 0; j < ROW_SIZE; j++)
		{
			...
			{
				gridMatrix[j][k] = stoi(GRID.substr(i, 2));
	...
	for (int j = 0; j < ROW_SIZE; j++)
	{
		for (int k = 0; k < COL_SIZE - 3; k++)
		{
			product = gridMatrix[j][k] * gridMatrix[j][k+1] * gridMatrix[j][k+2] * gridMatrix[j][k+3];
	...
	for (int j = 0; j < ROW_SIZE - 3; j++)
	{
		for (int k = 0; k < COL_SIZE; k++)
		{
			product = gridMatrix[j][k] * gridMatrix[j+1][k] * gridMatrix[j+2][k] * gridMatrix[j+3][k];
	...
	for (int j = 0; j < ROW_SIZE - 3; j++)
	{
		for (int k = 0; k < COL_SIZE - 3; k++)
		{
			product = gridMatrix[j][k] * gridMatrix[j+1][k+1] * gridMatrix[j+2][k+2] * gridMatrix[j+3][k+3];
